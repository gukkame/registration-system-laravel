...
export default {
    ...
    methods: {
        ...
        searchUser() {
            ...
            const normalizedGivenName = this.removeDiacritics(
                this.givenName.toLowerCase()
            );
            const filteredArray = this.users.filter(
                (obj) =>
                    this.removeDiacritics(obj.last_name.toLowerCase()).includes(
                        normalizedGivenName
                    ) ||
                    this.removeDiacritics(
                        obj.first_name.toLowerCase()
                    ).includes(normalizedGivenName)
            );
